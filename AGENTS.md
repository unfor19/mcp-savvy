...
- Never publish the CLI from a package directory or pass an npm OTP through
  Make, argv, or environment variables. Use `make publish-cli`, which requires
  a clean committed tree, builds from an isolated Git archive after a frozen
  lifecycle-script-free install, runs verification with a temporary credential-
  free home, consumer-tests the exact tarball on Node 20, and exposes npm auth
  only to `npm publish` for that inspected artifact.
...
- License: MIT
- Node baseline: 20
- Credential-bearing endpoints: HTTPS, except exact loopback HTTP for local
  development; OIDC discovery issuer matching is exact and redirects fail closed
