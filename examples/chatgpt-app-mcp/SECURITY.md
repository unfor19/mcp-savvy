...
controls to keep the user's balance out of model-visible fields when
the MCP Apps host honors app-only visibility and `_meta` isolation.
...
### 1.1 Privacy boundary — no balance in model-visible fields
...
| Widget endpoint      | Returns the actual `{ balance, currency, formatted }` in app-only `_meta`; compatible hosts isolate it from model context.                    |
...
The host boundary is an explicit assumption: this repository cannot
force an MCP Apps host to hide app-only tools or `_meta` from its model.
The subject-, purpose-, expiry-, and single-use-bound reference remains
server-enforced even if a host violates that presentation contract.
...
| API Gateway Cognito authorizer | Validates the JWT and enforces the `balance.read` OAuth scope before Lambda invocation.                         |
| Lambda claim consumption       | Consumes authorizer claims and fails closed when the authenticated `sub` is absent.                              |
| Re-auth on widget              | Widget POST carries the same Bearer token through the same API Gateway authorization boundary.                   |
...
| DynamoDB PITR                | Point-in-time recovery enabled on all four tables.                                     |
