...
widget renders the actual amount (e.g. `$12,340.55`); the model
...
| `McpSavvyChatGptAppData`    | Three DynamoDB tables (`customer_data`, `secure_view_refs`, `audit_log`) + seed Lambda.  |
...
The model never sees the amount because:
...
- The actual balance lands in `_meta`, which the host delivers to the iframe only — never the conversation transcript.
