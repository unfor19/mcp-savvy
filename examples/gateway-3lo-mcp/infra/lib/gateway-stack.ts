...
export interface GatewayStackProps extends cdk.StackProps {
     ...
     * `${targetName}___${operationId}` (e.g. `github___getAuthenticatedUser`).
     * Defaults to `github`.
     */
 ...
 *   2. Agent calls a `github___*` tool → AgentCore Identity returns
...
export class GatewayStack extends cdk.Stack {
    ...
    constructor(scope: Construct, id: string, props: GatewayStackProps) {
        ...
        new cdk.CfnOutput(this, 'TargetName', {
            value: targetName,
            description:
                'OpenAPI target prefix — operations surface as ${targetName}___${operationId}',
        });
