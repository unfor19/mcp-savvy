...
/**
 * Cognito stack for the mcp-savvy gateway-lambda-mcp demo.
 *
 * Same shape as `examples/minimal-mcp`'s Cognito stack — wraps the
 * `@mcp-savvy/cdk` `CognitoAuth` construct so the example shows
 * users how to compose it inside their own CDK app. Re-exports the
 * `CognitoAuth` instance as `auth` so the gateway stack can wire
...
export class CognitoStack extends cdk.Stack {
    ...
    constructor(scope: Construct, id: string, props: CognitoStackProps) {
        ...
        this.auth = new CognitoAuth(this, 'Auth', {
            ...authProps,
            userPoolName: 'mcp-savvy-gateway-demo',
        });
