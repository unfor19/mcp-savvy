#!/usr/bin/env node
/**
 * CDK app entrypoint for the mcp-savvy gateway-lambda-mcp demo.
 *
 * Three deployments, controlled by `MCP_SAVVY_DEMO_IDP`:
 *
 *   cognito (default)              greenfield Cognito user pool
 *     - McpSavvyGatewayDemoCognito  (provisions pool + clients + Hosted UI)
 *     - McpSavvyGatewayDemoGateway  (AgentCore Gateway gated by Cognito JWT,
 *                                    Lambda target with two demo tools)
 *
 *   imported-cognito               existing Cognito pool you operate
 *     - McpSavvyGatewayDemoGateway only (no Cognito stack — points at
 *                                       the pool referenced by the
 *                                       MCP_SAVVY_SHARED_* env vars)
 *
 *   entra                          Microsoft Entra ID v2 tenant
 *     - McpSavvyGatewayDemoEntraGateway (no AWS-side IdP; Entra is
 *                                       the issuer; bound by `azp`
 *                                       not `client_id` per v2 quirk)
 *
...
if (idp === 'cognito') {
    const domainPrefix =
        process.env['MCP_SAVVY_GATEWAY_DEMO_DOMAIN_PREFIX'] ?? 'mcp-savvy-gw-demo';

    // MFA defaults to REQUIRED (TOTP-only). Pass MCP_SAVVY_GATEWAY_DEMO_MFA=off
    // to relax to OPTIONAL for demos where forcing TOTP enrollment is
    // unwelcome — every other deployment should leave this unset.
    const mfaRequired =
        (process.env['MCP_SAVVY_GATEWAY_DEMO_MFA'] ?? 'on').toLowerCase() !== 'off';

    const cognito = new CognitoStack(app, 'McpSavvyGatewayDemoCognito', {
        env,
        domainPrefix,
        mfaRequired,
    });

    new GatewayStack(app, 'McpSavvyGatewayDemoGateway', {
        env,
    ...
    new GatewayStack(app, 'McpSavvyGatewayDemoEntraGateway', {
        env,
        gatewayName: 'mcp-savvy-gw-demo-entra',
