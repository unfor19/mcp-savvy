...
import { AuthError, isSecureEndpointUrl } from '@mcp-savvy/core';
...
export async function completeGatewaySession(
    ...
    if (!isSecureEndpointUrl(input.authorizationUrl)) {
        throw new AuthError(
            'AUTH_PROVIDER_ERROR',
            'authorization URL must use HTTPS or exact loopback HTTP',
        );
    }
    if (!isSecureEndpointUrl(input.completeSessionEndpoint)) {
        throw new AuthError(
            'AUTH_PROVIDER_ERROR',
            'complete-session endpoint must use HTTPS or exact loopback HTTP',
        );
    }
