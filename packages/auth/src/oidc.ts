...
import { AuthError, isHttpsEndpointUrl, isSecureEndpointUrl } from '@mcp-savvy/core';
...
export async function fetchOIDC(issuer: string, fetcher: Fetcher): Promise<OIDCEndpoints> {
    if (!isSecureEndpointUrl(issuer)) {
        throw new AuthError(
            'OIDC_DISCOVERY_FAILED',
            'OIDC issuer must use HTTPS or exact loopback HTTP',
        );
    }
    ...
    if (!doc.authorization_endpoint || !doc.token_endpoint || !doc.issuer) {
        throw new AuthError(
            'OIDC_DISCOVERY_FAILED',
            `OIDC discovery doc missing required fields (${url})`,
        );
    }
    if (doc.issuer !== issuer) {
        throw new AuthError('OIDC_DISCOVERY_FAILED', 'OIDC discovery issuer mismatch');
    }
    const secureEndpoints =
        !isSecureEndpointUrl(doc.authorization_endpoint) ||
        !isSecureEndpointUrl(doc.token_endpoint);
    const remoteIssuerDowngrade = isHttpsEndpointUrl(issuer) && (
        !isHttpsEndpointUrl(doc.authorization_endpoint) ||
        !isHttpsEndpointUrl(doc.token_endpoint)
    );
    if (secureEndpoints || remoteIssuerDowngrade) {
        throw new AuthError(
            'OIDC_DISCOVERY_FAILED',
            'OIDC discovery endpoints must use HTTPS or exact loopback HTTP',
        );
    }
