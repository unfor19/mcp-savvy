...
describe('loadConfig', () => {
    ...
    it('rejects plaintext remote credential endpoints', () => {
        expectConfigError({ ...REQUIRED, MCP_SAVVY_REMOTE_URL: 'http://example.com/mcp' }, 'MCP_SAVVY_REMOTE_URL');
        expectConfigError({ ...REQUIRED, MCP_SAVVY_OIDC_ISSUER: 'http://idp.example.com' }, 'MCP_SAVVY_OIDC_ISSUER');
        expectConfigError({ ...REQUIRED, MCP_SAVVY_COMPLETE_SESSION_URL: 'http://api.example.com/complete' }, 'MCP_SAVVY_COMPLETE_SESSION_URL');
    });

    it('preserves exact loopback HTTP development endpoints', () => {
        const cfg = loadConfig({
            ...REQUIRED,
            MCP_SAVVY_REMOTE_URL: 'http://127.0.0.1:3000/mcp',
            MCP_SAVVY_OIDC_ISSUER: 'http://localhost:8080',
            MCP_SAVVY_COMPLETE_SESSION_URL: 'http://[::1]:4000/complete',
        });
        expect(cfg.remoteUrl).toBe('http://127.0.0.1:3000/mcp');
    });
