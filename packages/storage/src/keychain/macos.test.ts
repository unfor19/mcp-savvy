...
describe('set', () => {
    it('deletes any prior entry then adds a new one', () => {
        const r = recordingRunner();
        const k = new MacOSKeychain({
            service: SERVICE,
            account: ACCOUNT,
        ...
        expect(r.calls[0]?.args[0]).toBe('delete-generic-password');
        expect(r.calls[1]?.args[0]).toBe('add-generic-password');
        expect(r.calls[1]?.args).not.toContain('payload');
        expect(r.calls[1]?.input).toBe('payload\n');
    });

    it('tolerates the delete throwing (no prior entry)', () => {
        const r = recordingRunner();
        let n = 0;
        r.setRunImpl(() => {
            n += 1;
