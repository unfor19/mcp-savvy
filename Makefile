...
# Load .env for operator commands, but never inside the isolated release build.
ifneq ($(RELEASE_ISOLATED),1)
ifneq (,$(wildcard .env))
    include .env
    export
endif
...
publish-cli-pack: ## Build, pack, and inspect the CLI in an isolated snapshot (no upload)
	$(NODE) scripts/release-cli.mjs

publish-cli: ## Isolate, verify, pack, inspect, then publish that exact tarball interactively
...
example-gateway-3lo-bootstrap: ## CDK bootstrap the account/region for gateway-3lo-mcp (run once)
	...
	AWS_PROFILE=$(AWS_PROFILE) AWS_REGION=$(AWS_REGION) $(GW_3LO_CDK) bootstrap \
