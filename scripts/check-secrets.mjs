...
const GENERIC_PATTERNS = [
    ...
    { name: 'OpenAI-style API key', pattern: /\bsk-[A-Za-z0-9]{40,}\b/ },
    {
...
function isScannable(file) {
    return !['.webp', '.png', '.jpg', '.gif'].some((extension) => file.endsWith(extension));
}
...
function indexEntries(repoRoot) {
    ...
    return output.toString('utf8').split('\0').filter(Boolean).flatMap((record) => {
        ...
        return match[2] === '0' && isScannable(match[3])
