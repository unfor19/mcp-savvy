...
## CDK construct source

The constructs below live in the private `@mcp-savvy/cdk` workspace
...
## Programmatic configuration use

The npm package exposes the stable CLI configuration helpers.
Runtime composition and the internal auth, storage, server, bridge, and CDK
workspaces remain implementation details and are not separate public packages.
...
const namespace = deriveNamespace(config.issuer, config.clientId);
console.log({ config, namespace });
```

The repository examples import `@mcp-savvy/*` through pnpm workspace
links. Those imports are source-development contracts, not npm
installation instructions for the public CLI package.
