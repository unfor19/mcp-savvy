...
| `get_credit_balance_actual` | Widget-only (`_meta.ui.visibility=['app']`). Redeems a single-use secure_view_ref and returns the actual balance via `_meta`, not `structuredContent`/`content`. Privacy assumes the host isolates app-only `_meta`. |
...
| `McpSavvyChatGptAppData` | 4 DynamoDB tables (customer_data, secure_view_refs, audit_log, branches) with AWS-managed encryption + PITR + deletion protection. |
